...
import pytest
from priority_queue_with_heap import PriorityQueue
...
def test_insert_correctly_sorts_list_with_no_dups(test_priority_q_no_dups):
    ...
    result = []
    for item in test_priority_q_no_dups.heap._list:
...
def test_pop_maintains_heap(test_priority_q_dups):
    assert test_priority_q_dups.pop() == 'test1'
    assert test_priority_q_dups.pop() == 'test2'
    assert test_priority_q_dups.pop() == 'test3'
    assert test_priority_q_dups.pop() == 'test9'
    assert test_priority_q_dups.pop() == 'test10'


def test_pop_one_item_works():
    test_pq = PriorityQueue()
    test_pq.insert(10, "test")
    assert test_pq.pop() == 'test'
    with pytest.raises(IndexError) as error:
        test_pq.pop()
    assert 'The queue is empty' in str(error.value)
